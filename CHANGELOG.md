...
- **标题/正文中含嵌套方括号的链接无法渲染**（如 `### [P3195 [HNOI2008] 玩具装箱](https://www.luogu.com.cn/problem/P3195)`）：行内链接正则是 `[^\]]+`，label 不允许含 `]`，洛谷标准题号格式（内套 `[HNOI2008]`）整链匹配失败原样泄漏。现链接 label 与图片 alt 均**容忍一层平衡方括号**（与 URL 组早已容忍一层平衡圆括号同构），普通链接/同行多链接/任务列表内复杂链接行为不变
- **左栏快速滚动后，右栏惯性拖着左栏继续向下**：v1.1.0 的程序化滚动抑制是固定 700ms 上限——长距离 smooth 动画超过 700ms 后抑制失效，仍在运行的动画尾事件被误当用户滚动回传，编辑器被 `revealRange` 拖着追随预览的惯性继续爬。双保险修复：①滚轮密集（<90ms）时正向同步直接 `auto` 跳变，不再积累惯性——惯性从源头消失；②抑制改为**事件看门狗**：动画事件还在流就持续抑制，停止 170ms 才释放，动画多长都压制得住
